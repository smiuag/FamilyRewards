...
export default function AdminStatsClient() {
  ...
  const weeklyData = members.map((user) => {
    ...
    const total = taskInstances.filter((ti) => ti.userId === user.id).length;
    return { user, completed, total };
  });

  const approvedClaims = claims.filter((c) => c.status === "approved");
  const pendingClaims = claims.filter((c) => c.status === "pending");
  const totalPoints = users.reduce((a, u) => a + u.pointsBalance, 0);
      ...
      <div className="grid lg:grid-cols-2 gap-6">
                  ...
                  <div key={user.id}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-sm font-bold text-primary">{user.pointsBalance.toLocaleString()}</span>
                      </div>
                    </div>
        ...
        <Card className="shadow-sm">
          ...
          <CardContent className="space-y-3">
            {weeklyData.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4">Sin datos de tareas aún</p>
            ) : (
                ...
                return (
                  <div key={user.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-medium">{user.name}</span>
                        <span className="text-xs text-muted-foreground">{completed}/{total} completadas</span>
                      </div>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {approvedClaims.map((claim) => {
                ...
                return (
                  <div key={claim.id} className="flex items-center gap-2 bg-green-50 text-green-700 px-3 py-2 rounded-xl text-sm">
